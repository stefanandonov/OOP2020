...
public:
  VirtualServer (char * IP = "127.1.0.0", char * name = "uknown", 
    int cores = 4, int bandwith = 1) : Server (IP, name) {
    this->cores = cores;
  ...
  VirtualServer (const VirtualServer & vs) : Server(vs) {
    this->cores = vs.cores;
    this->bandwith = vs.bandwith;
  }

  double total_price () {
      //основната цена + хард_диск_простор * 0.5 + рам_меморија * 20.
    return cores*5 + bandwith*10;
  }

  VirtualServer & operator += (int cores) { //vs+=5 (jadra ke se zgolemi za 5)
    this->cores += cores;
    return *this; 
  }

  VirtualServer operator ++ (int) {
    VirtualServer copy(*this);
    bandwith++;
    return *this;
  }

  friend ostream & operator << (ostream & out, const VirtualServer & vs) {
    /*[име на хост]: [IP адреса]
    [број на јардра], [количина на потрошен интернет]
    */

    return out<<vs.name<<": "<<vs.IP<<endl<<vs.cores<<", "<<vs.bandwith<<endl;
  }
};
...
double totalCost (Server ** servers, int n) {
  double sum = 0.0;
  for (int i=0;i<n;i++)
    sum+=servers[i]->total_price();
  return sum;
}

Server & biggestInvoice (Server ** servers, int n) {
  Server * max = servers[0];
  for (int i=1;i<n;i++)
    if (*servers[i]>*max)  ////preoptovareno operator >= ako stanuva zbor za klasi 
      max = servers[i];
  return *max;
} 
